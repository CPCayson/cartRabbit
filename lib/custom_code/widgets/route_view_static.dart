...
class _RouteViewStaticState extends State<RouteViewStatic> {
  ...
  late Future<bool> markersLoaded;
  ...
  Future<bool> generateMarkers() async {
    if (widget.positions.isEmpty) {
      return true;
    }
    for (final location in widget.positions) {
      print("TESTAROO ADDING MARKERS");
      final http.Response response = await http.get(Uri.parse(
          location.imageUrl.isEmpty
              ? widget.defaultImageUrl!
              : location.imageUrl));
      final icon =
          BitmapDescriptor.fromBytes(response.bodyBytes, size: Size(24, 24));
